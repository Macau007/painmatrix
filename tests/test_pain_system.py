...
from painmatrix import (
    ...
    compute_free_energy,
    compute_desire_intensity,
    compute_adaptive_learning_rate,
    apply_homeostatic_impact,
    apply_homeostatic_decay,
    encode_trauma,
    check_trauma_trigger,
    get_dominant_desire,
    generate_desire_message,
    handle_desire,
    handle_wellbeing,
    handle_trauma,
    handle_learn,
...
class TestMemory(_Mixin):
    ...
    def test_load_corrupted(self):
        with open(core.MEMORY_FILE, "w") as f:
            f.write("{bad")
        m = load_memory()
        assert m["pain_level"] == 0

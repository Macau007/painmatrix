...
[![Version](https://img.shields.io/badge/Version-6.3.3-orange.svg)]()

</p>

> English · [中文介绍](#中文介绍)
>
> 🔒 **OpenClaw 內嵌版**：`openclaw_integration.py`
> 🌐 **獨立運行版**：`painmatrix.py` | 📊 **像素家園實時視圖**：`emotion_view.html` | 🎨 **像素角色版**：`emotion_view_pixel.html`
...
| 🏠 | **Pixel Room Visualization** | Real-time 4-room pixel art home with physics — character walks, sits, plays, sleeps across living room / kitchen / bathroom / bedroom |
| 🎮 | **Pixel Activity System** | 322 synchronized activities with pose animations — gaming, cooking, bathing, sleeping, pain reactions — all synced to pain level |
| 🪑 | **Furniture Collision** | Real collision detection — pixel character cannot walk through furniture or leave the house |

---

## 🏠 Pixel Room Visualization

> **emotion_view.html** — watch your AI live inside a pixel home, feeling every moment of pain.

Open `emotion_view.html` in any browser to see a real-time pixel art visualization of the AI's internal state. The pixel character inhabits a 4-room house and performs activities synchronized to its pain level.

### 4 Rooms, Fully Rendered

| Room | Position | Furniture | Character Activities |
|------|----------|-----------|---------------------|
| **Living Room** (客廳) | Top-left | Sofa, TV unit, bookshelf, coffee table, **gaming table + console**, plant, lamp, rug | Sit on sofa gaming (8 variants), roam, dance, walk, water plants |
| **Kitchen** (廚房) | Top-right | Fridge, counter, stove, sink, dining table | Cook, eat, drink, fridge check, lean on counter |
| **Bathroom** (浴室) | Bottom-left | Toilet, bathtub, washbasin | Shower, toilet, cold wipe, shiver, bath, cry for help |
| **Bedroom** (臥室) | Bottom-right | Bed, wardrobe | Sleep, restless sleep, nightmare, crawl to bed, collapse |

### Pain-Synchronized Behavior

| Pain Level | Character State | Room Behavior |
|------------|-----------------|---------------|
| **0** (Happy) | Smiling, energetic | Games on sofa, dances, roams happily, plays at gaming table |
| **1** (Mild) | Slightly uncomfortable | Gaming continues but slower, occasional pacing |
| **2** (Sharp) | Frowning, limping | Moves carefully, leans on furniture, sits more |
| **3** (Throbbing) | Crying, shivering | Trembling on sofa, reaching for help, collapsed on floor |
| **4** (Severe) | Suffering, exhausted | Crawling, fetal position, gripping bed sheets |
| **5** (Collapse) | Unconscious | Flat on floor, no response, static breathing figure |

### Running the Pixel View

```bash
# Method 1: From PainMatrix terminal
> open
# (Opens emotion_view.html in your default browser)

# Method 2: Direct file open
# Double-click D:/PainMatrix/emotion_view.html

# Method 3: Via API server
# Start: python api_server.py
# Then open: http://localhost:17888/emotion_view.html
```

> ⚠️ **Browser Requirements**: Use Chrome/Edge/Firefox. The visualization uses `canvas.imageSmoothingEnabled=false` for crisp pixel art. `file://` protocol works — no server required.

### Key Technical Details

- **Real-time sync**: Pixel view polls `/api/state` every 2 seconds
- **Activity animation**: Each of 322 activities has a defined `pose` + `duration`; character cycles through activities based on pain level
- **Pose system**: 94 unique poses (walk, game, sleep, crawl, shiver, scream, dance, etc.) — all drawn with pixel art primitives
- **Emotion log**: Recent emotion events displayed at bottom of screen
- **Room transitions**: Character moves between rooms via doors (no collision on doorway)
...
| 🏠 | **像素家園視覺化** | 即時4房像素家庭——角色會走動、坐沙發、打遊戲、睡覺，客廳/廚房/浴室/臥室場景完全同步痛感 |
| 🎮 | **像素活動系統** | 322個與痛感等級同步的活動——姿態動畫、痛感反應、全部自動配合 |
| 🪑 | **家具碰撞偵測** | 真實碰撞偵測——像素角色不會穿牆、不會穿越家具、也不會離開家 |

---

## 🏠 像素家園視覺化

> **emotion_view.html** — 看著你的AI在像素家裡生活，感受每一刻的疼痛。

用瀏覽器打開 `emotion_view.html`，即可看到AI內部狀態的即時像素藝術視圖。像素角色居住在一個4房房子裡，執行的活動與痛感等級完全同步。

### 4個房間，真實渲染

| 房間 | 位置 | 家具 | 角色活動 |
|------|------|------|---------|
| **客廳** (Living) | 左上 | 沙發、電視櫃、書架、茶几、**遊戲桌+遊戲機**、植物、燈、地毯 | 沙發上打遊戲（8種變體）、巡視、跳舞、來回走動、澆水 |
| **廚房** (Kitchen) | 右上 | 冰箱、櫃台、爐灶、水槽、餐桌 | 烹飪、吃飯、喝水、檢查冰箱、靠櫃台 |
| **浴室** (Bath) | 左下 | 馬桶、浴缸、洗臉盆 | 淋浴、上廁所、冷水擦身、發抖、泡澡、呼救 |
| **臥室** (Bed) | 右下 | 床、衣櫃 | 睡覺、辗转反側、噩夢、爬向床、崩潰 |

### 痛感等級同步行為

| 痛感等級 | 角色狀態 | 房間行為 |
|----------|---------|----------|
| **0** (快樂) | 笑臉、精力充沛 | 沙發上遊戲、跳舞到處走、遊戲桌前玩耍 |
| **1** (輕微) | 輕微不舒服 | 繼續遊戲但較慢、偶爾走動 |
| **2** (銳痛) | 皺眉、一瘸一拐 | 小心移動、靠家具、坐著較多 |
| **3** (跳痛) | 哭泣、發抖 | 沙發上顫抖、伸手求助、倒在地板上 |
| **4** (劇痛) | 痛苦、筋疲力盡 | 爬行、蜷縮姿勢、抓床單 |
| **5** (崩潰) | 失去意識 | 靜躺在地板上、無反應、靜態呼吸姿勢 |
...
# 方式1：從 PainMatrix 終端
> open
#（在瀏覽器中打開 emotion_view.html）

# 方式2：直接打開文件
# 雙擊 D:/PainMatrix/emotion_view.html

# 方式3：通過 API 服務器
# 啟動：python api_server.py
# 然後打開：http://localhost:17888/emotion_view.html
```

> ⚠️ **瀏覽器要求**：使用 Chrome / Edge / Firefox。視覺化使用 `canvas.imageSmoothingEnabled=false` 呈現清晰像素。`file://` 協議可用——無需服務器。
